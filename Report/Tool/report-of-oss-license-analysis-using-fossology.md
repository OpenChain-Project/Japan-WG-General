...
Open Source Software (OSS) is one of the most important things in developing large systems. Therefore, to understand OSS and liceses is critical. OpenChain project hosted by the Linux Foundation is one of the activities to build trust over software supply chain. 
...
(This report is written for Japanese readers, so that we use Japanese environment.) 
